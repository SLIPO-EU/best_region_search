...
import eu.slipo.athenarc.brs.Grid;
...
public class BRSIndexProgressiveDivExhaustive extends BRS<POI> {

	private double decayConstant;
	private GeometryFactory geometryFactory;
	private UtilityScoreFunction utilityScoreFunction;
	// private long overallStartTime, resultEndTime;

	public BRSIndexProgressiveDivExhaustive(double decayConstant, UtilityScoreFunction utilityScoreFunction) {
		super();
		this.decayConstant = decayConstant;
		this.utilityScoreFunction = utilityScoreFunction;
	}

	@Override
	public List<SpatialObject> executeBRS(List<POI> pois, double eps, int k,
	...
	private void processBlock(Block block, double eps, ScoreFunction<POI> scoreFunction, PriorityQueue<Block> queue,
		...
		if (block.type == Block.BLOCK_TYPE_REGION) {
			inspectResult(block, eps, topk, queue);
		} else {
			List<Block> newBlocks = block.sweepExhaustive();
			queue.addAll(newBlocks);
		}
	}

	private void inspectResult(Block block, double eps, List<SpatialObject> topk, PriorityQueue<Block> queue) {
		...
		SpatialObject result = new SpatialObject(block.envelope.centre().x + ":" + block.envelope.centre().y, null,
				null, block.relevanceScore, geometryFactory.toGeometry(e));
		result.setAttributes(new HashMap<Object, Object>());
		result.getAttributes().put("coveredPoints", block.pois);
		result.getAttributes().put("relevanceScore", block.relevanceScore);

		double previousScore = block.utilityScore;
		double newScore = utilityScoreFunction.computeDiscountedRelevance(topk, result, decayConstant);

		if (newScore < previousScore) {
			block.utilityScore = newScore;
			queue.add(block);
		} else {
			...
			System.out.println("Result found: " + result.getGeometry().getCentroid().getX() + " : "
					+ result.getGeometry().getCentroid().getY() + " @ " + result.getScore() + " Results so far: "
					+ topk.size());
			// System.out.println("Added result with utility score: " +
			// result.getScore());
		}
	}
